...
\begin{figure}
    ...
    \caption{$H_{\rm bc}$ solutions shown at three line center optical depths, $\tau_0=10^5, 10^7,$ and $10^9$, for photon source frequencies $\sigma_s=0.0, \tau_0, 2\tau_0,$ etc. Thicker lines indicate larger optical depth, and darker lines indicate a photon source frequency further from line center. The axes are scaled to show that the size of the corrective factor agrees with the predicted $(a\tau_0)^{-1/3}$ scaling.}
    \label{fig:xinit}
\end{figure}

Fig. \ref{fig:xinit} shows how the boundary term scales with both optical depth and photon source frequency. The photon initialization frequency is shifted in integer multiples of $\tau_0$. For clarity, only the positive side of the spectrum is shown. From Eq. \ref{eq:hbc_scaling}, the size of the correction should become smaller with larger optical depths, following a $(a\tau_0)^{-1/3}$ scaling. This factor has been scaled out of the figure such that solutions for different optical depths should show close agreement in scale on the figure if the relation holds, even while changing the source frequency of the photons. Indeed, the solutions converge as the optical depth becomes larger. The remaining discrepancy present is due to the width of the Doppler core with respect to the peak of the spectrum; at lower depths, this causes our assumptions about the functional form of the line profile to break down.
...
\be
\gamma_{\rm guess} &=& \frac{a\gamma_{j-1} - \gamma_{j+1}}{a - 1}
\ee
where
\be
a &=& \left(\frac{f_{j} - f_{j-1}}{f_{j} - f_{j+1}}\right)\left(\frac{\gamma_{j+1}-\gamma_{j}}{\gamma_{j-1}-\gamma_{j}}\right).
\ee
...
\begin{figure}
    ...
    \caption{Eigenfunctions $J_{nm}(x)$ for the lowest-order spatial eigenmode $n=1$, and a range of frequency eigenmodes up to $m=100$. Their oscillatory forms have varying amplitudes which sum to create the final form of the mean intensity. The largest contribution always comes from the ($n=1, m=1$) lowest-order eigenfunction. \todo{Show dimensional factor out front of the eigenfunctions to explain why they are so small. Dimensional analysis of the mean intensity itself. Radius, energy (set to 1), delta, and dimensionless parameters a and tau0.}}
    \label{fig:jsoln}
\end{figure}
...
\ee
Integrating over $\nu$ gives unity if the sum rule in Eq. \ref{eq:sumrule} is true. In Fig. \ref{fig:steadystate}

\begin{figure}
    \centering
    \includegraphics{steadystate.pdf}
    \caption{Comparison of steady-state and time-integrated spectra for 20 spatial eigenmodes and 500 frequency eigenmodes. The accuracy of the red scatter points is limited by the number of frequency eigenmodes summed. The accuracy of the core behavior for the orange solid curve and red scatter points is limited by the number of spatial eigenmodes summed, which is why both solutions fail in the core. \todo{Steady state, Steady state (partial sum), Fluence of time-dependent solution, start at 8 and go to 30. Make the red line thinner and plot it in the background.}}
    \label{fig:my_label}
\end{figure}
