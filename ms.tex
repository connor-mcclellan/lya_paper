...
\ee
at $r=R$. 
%TODO: Remove all references to \vec{x}_s in the paper. Specify once that it's set to zero.
...
We now wish to reconstruct the specific mean intensity $J(r, \sigma, t)$. While one might expect this could be expressed as a sum over eigenmodes, the analysis presented in Appendix \ref{app:wkb} suggests this ansatz is incomplete in the case where $x_s \neq 0$ and the solution is asymmetric about the line center. This treatment roughly agrees with Monte Carlo results for $x_s=0$ based on numerical calculations of this result. 
...
\begin{figure}
    ...
    \caption{Time-integrated spectrum $P(x)$ vs. $x$. The steady-state and time-integrated spectra for $n=1, ..., 20$ and $m=1, ..., 500$ are shown with $x_s=0$, $a = 4.72\times 10^{-4}$ and $\tau_0=10^7$.}
...
In this appendix, we will find a WKB solution for the differential equation Eq. \ref{eq:wkb_differential_eqn} subject to the boundary conditions in Equations %TODO: Ref 28 and 29. 
We will find this solution for the case $\sigma_s = 0$.

% TODO: All the sigma in this appendix are presumed to be positive, but we've just assumed thta. It happens to be symmetric in this case;

Near line center, this has two independent solutions which can be written
\be \label{eq:wkb_J}
...
%The eigenfunctions themselves are symmetric
% ^^^ Put this as the first thing we're saying. 

% Summarize the necessary points of the sigmas=0 solution 
