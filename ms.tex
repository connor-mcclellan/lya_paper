...
TODO: Don't use s in these equations. Use damping rate omega here.
We employ an expansion in terms of spherical Bessel functions in $r$ and a Laplace expansion with the imaginary frequency variable $s=-i\omega$. The eigenvalues $s$ are real and negative for damped solutions to the time-dependent equation. The expansion for $J(r, \sigma, t)$ is
...
\be
 \left( \frac{3k\phi}{c}i\omega  -   \kappa_n^2 \right) J(n,\sigma,\omega)  &+& \left( \frac{k}{\Delta} \right)^2 \frac{d^2J(n,\sigma,\omega)}{d\sigma^2} = \frac{2\kappa_n^2}{R} (-1) \frac{\sqrt{6}}{4\pi} \frac{kE}{\Delta^2} \frac{1}{4\pi} \delta(\sigma - \sigma_s).
\ee
...
Two numerical integrations are performed inward from large $\pm \sigma$ to the source $
\sigma_s$. The starting values of J are set to be $J_1$ and $J_2$ for the rightward and leftward integrations, respectively. Choices of these starting values are adjusted to ensure that the matching conditions at the source are met.

Separating the real and imaginary parts of the solution $J = J_r + i J_i$, we have
...
The starting conditions are $J_{1,r}$, $J_{1,i}$, $J_{2,r}$, $J_{2,i}$.
The rightgoing and leftgoing integrations then produce $J_{-,r}$, $J_{-,i}$, $J^\prime_{-,r}$, $J^\prime_{-,i}$,  $J_{+,r}$, $J_{+,i}$, $J^\prime_{+,r}$, $J^\prime_{+,i}$, where the prime means $\partial/\partial \sigma$. The matching conditions are then
\be
J_{-,r} & =& J_{+,r} \\
J_{-,i} & =& J_{+,i} \\
...
For small $t$ the solution is nearly a delta function in radius TODO

It is expected that the time-integrated spectrum of the response to an impulse will agree well with the solution for the steady-state spectrum (Eq. TODO: Steady state eqn). We first evaluate the time-dependent response using our eigenfrequency sweeping method. Once the probabilities $P_{nm}(\sigma)$ are calculated as above, the escape time distributions for all photon frequencies are calculated by summing over the frequency eigenfunctions $P_{nm} \exp(s_{nm}t)$. Integrating $P_{nm}$ with respect to $t$, we obtain
