...
The solution of the radiative transfer equation for resonant scattering of Lyman $\alpha$ photons is explored in spherical geometry. A monochromatic source of photons is assumed at the center of a uniform sphere of constant gas density. The intensity within the sphere and at its surface is found in the limit of large line-center optical depth using the Eddington approximation for the angular dependence. Two specific limits are studied: a steady source and an impulsive source. For the steady-state problem, the solution can be represented as a sum of two terms: a previously-known analytic solution of the equation with the mean intensity $J=0$ at the surface, and a novel, semi-analytic solution which enforces the correct zero-ingoing intensity at the surface. This solution is compared to that of the Monte-Carlo method, which is valid at arbitrary optical depth. It is shown that the size of the correction depends on the line center optical depth, and may explain discrepancies in previous studies that used a $J=0$ boundary condition. For the impulsive problem, the time, spatial, and frequency dependence of the diffusion equation are expressed using an eigenfunction expansion and solved numerically to characterize the escape time distribution and spectra of photons initialized at the center of the sphere. It is shown that the lowest-order eigenfrequency of the semi-analytic calculation agrees well with the decay constant of the Monte Carlo escape time distribution at line-center optical depths above ${\sim} 10^7$. A fitting function for the distribution is found which can be sampled to accelerate Monte Carlo radiation transfer calculations at large optical depths.
...
Numerical simulations can be used to understand the dynamics of these irradiated exoplanet atmospheres. The extreme optical depths at \lya line center impose a large computational cost for solving radiative transfer with Monte Carlo methods directly coupled with magnetohydrodynamic simulations. The number of scatterings a photon undergoes is proportional to the line center optical depth, $\tau_0$.  Near the base of the atmosphere, the line center optical depth is ${\sim}10^9$, so most of the time is spent following photons in these cells. A method that can treat these zones incorporating a direct solution rather than traditional Monte Carlo has the potential to greatly accelerate the calculation. 
...
\label{eq:evalue}
\ee
where $2B$ is the slab optical depth at line center, $\phi$ is the line profile, and $\Delta$ is the Doppler width. The key point is that the line profile depends on one of the coordinates, frequency, and this causes the eigenvalues of the separation constant to depend on frequency. Thus, the separation constant is not constant, and the solution does not satisfy the Poisson equation since the frequency derivatives will act on the separation ``constant", giving extra terms. In the limit of large optical depth $B$, they approximate the eigenvalues as $\lambda_n B \simeq \pi (n-1/2)$. However, this solution gives zero mean intensity at the surface. Their Eq. 34 subsequently allowed the separation constant to have a small deviation from the above expression, which was explicitly frequency dependent. This allowed a nonzero intensity at the surface, but at the cost of rendering the separation of variables assumption invalid. Our treatment, using the correct boundary condition, quantifies the errors in their ansatz.
...
\ee
where $J$ is the mean intensity, $L$ is a luminosity input, $\sigma$ represents a change of variables in photon frequency (Eq. \ref{eq:change_of_variables}), and $\sigma_s$ is is the photon frequency of the source. $\sigma_s$ is interchangeable with $x_s$ and $\nu_s$ in Doppler units or Hz, respectively. The spatial variable is $\vec{x}$ and $\vec{x}_s$ is the position of the source. The boundary condition that there is no incoming intensity at the surface \citep{1986rpa..book.....R} can be written as
\be
J & = & \sqrt{3} H
\label{eq:bc}
\ee
...
\label{eq:H0}
\ee
Again $J_0 \gg H_0$, except near the $r=R$, where it goes to zero. The flux at $r=R$ can be written
\be
\nonumber
...
\ee
Here, $J_0$ and $H_0$ are given in Equations \ref{eq:J0} and \ref{eq:H0}. The additional term $J_{\rm bc}$ must then be a solution of the homogeneous equation
\be \label{eq:diffeq}
...
\label{eq:bc2}
\ee
Inserting a frequency dependence $J_{\rm bc} \propto e^{is\sigma}$, for ``wavenumber" $s$, gives the equation for modified spherical Bessel functions of the first kind, $i_0(z)=\sinh(z)/z$. The solution can then be represented as
\be
J_{\rm bc}(r,\sigma) & = & 
...
In Figure \ref{fig:sol_mc_residual_0}, the Monte Carlo escape probability as a function of frequency is shown with that of the solutions $H_{\rm d}$, $H_0$, and $H_{\rm 0 + bc} = H_0 + H_{\rm bc}$ for an optical depth of $\tau_0 = 10^7$ and photons emitted at line center $\rm x_s = 0$.  The $H_{\rm bc}$ term is negative at the peak of the spectrum and positive in the line wing such that, when added to $H_0$, it adequately corrects for the apparent excess of flux in the peaks of the spectrum. The solution with the correct frequency-dependent boundary condition enforced, $H_{0 + \rm bc}$, has considerably lower residuals to Monte Carlo results than the other solutions, especially in the line wing. The boundary term corrects the slight deficit of $H_0$ in the line wings, further improving agreement with the numerical result. Note that the residuals to the $H_0$ solution are a close match to the $H_{\rm bc}$ term, since the Monte Carlo represents the ``true'' solution, $H$, and $H_{\rm bc} = H - H_0$. It is evident that the divergent solution $H_{\rm d}$ fails in the line wings.
...
Figure \ref{fig:sol_mc_xinit_lowtau} shows emission away from line center at the same values of $x_s$ as in Figure \ref{fig:sol_mc_xinit}, but for $\tau_0=10^6$ rather than $10^7$. The difference between the left and right side of the escaping spectrum is substantial at lower optical depths, since a change in $x_s$ of 6 is a larger fraction of the spectral peak location $(a\tau_0)^{1/3}$ for lower $\tau_0$. It is clear from the figure that as $x_s$ moves further from the line core, the spectrum becomes more strongly peaked in frequency. Additionally, since the material is increasingly optically thin in the wing, we expect there to be a stronger disagreement between the Monte Carlo and the analytic solution as our derivation assumed large optical depths.
...
\ee
We employ an expansion in terms of spherical Bessel functions in $r$ and transform the time dependence into an imaginary frequency variable $\omega$. The expansion for $J(r, \sigma, t)$ is
\be
...
\ee
Eq. \ref{eq:jrsigmat_expansion} is the standard specific mean intensity $dE/dA dt d\nu$. Though $J(r, \sigma, t)$ is written as a function of the photon frequency variable $\sigma$, it should be integrated over $d\nu$ to preserve its normalization. Eq. \ref{eq:jnsigmaomega} is in the same units, but with an additional unit of $t$ in the numerator as a result of the expansion. Inserting Eq. \ref{eq:jnsigmaomega} for the eigenfunctions in Eq. \ref{eq:diffusion_eqn} and evaluating the integrals, we obtain
...
\ee
This cannot be solved analytically, so we turn now to a semi-analytic scheme to solve this equation for $J$. We start with a treatment of the boundary conditions. At $\sigma=\sigma_s$ continuity must be enforced,
...
\ee
At large values of $\sigma$ the line profile is small such that Eq. \ref{eq:diffusion_plugged_in} becomes
...
\ee
where a negative sign is taken for large $+\sigma$ and a positive sign is taken for large $-\sigma$. Two numerical integrations are performed inward from large $\pm \sigma$ to $\sigma_s$, isolating the solution that increases exponentially toward the line center. Initial values for integration are obtained by setting $J=1$ in Eq. \ref{eq:single_j_derivative}. This gives $J$ and $J'$ on either side of $\sigma_s$, where a prime indicates the derivative $\partial/\partial \sigma$. By enforcing the matching conditions, Eqs. \ref{eq:matching_condition_1} and \ref{eq:matching_condition_2}, the eigenfunctions $J(n, \sigma, \omega)$ are obtained for all photon frequencies $\sigma$. Since the eigenfunctions are linear in the starting conditions, only two integrations with different starting values are necessary.

Let us define the damping frequency to be $\gamma \equiv i\omega$, which is real-valued and positive for damped solutions to the time-dependent equation. There are certain values of $\gamma$ for which a resonant response is produced which we will call $\gamma_{nm}$. Near these $\gamma_{nm}$ poles, the eigenfunctions will have the form 
...
\ee
From Eq. \ref{eq:jrsigmat_expansion}, the contribution of a single pole to the total solution is
\be
J_{nm}(r,\sigma,t) & = &  -j_0(\kappa_n r)  \int \frac{d\gamma}{2\pi i} e^{-\gamma t}J(n,\sigma,-i\gamma)
\\ & \simeq & j_0(\kappa_n r)  \int \frac{d\gamma}{2\pi i} e^{-\gamma t} \frac{ J_{nm}(\sigma) }{\gamma - \gamma_{nm}}.
\ee
We close the integral along the imaginary $\gamma$-axis  with a semi-circle at infinity and apply the residue theorem. The contribution from the semi-circle is zero, so we obtain
\be
...
\ee
This ansatz captures the contributions from $n \times m$ simple poles, which seems to agree with Monte Carlo results for $x_s=0$ based on numerical calculations of this result. However, the analysis presented in Appendix \ref{app:wkb} suggests the ansatz is incomplete in the case where $x_s \neq 0$ and the spectrum is asymmetric.
...
\label{eq:waittime}
\ee
which is normalized to unity. For a sufficiently large number of spatial modes $n$ and frequency modes $m$, the result of this sum agrees with Monte Carlo escape time distributions. % Not sure if we want to say this yet? Maybe save for the escape time figure.
...
% Put a little wkb in the appendix? We could show the dispersion relation, for instnace. d^2Jdsigma^2, equation for discontinuity... here is the solution far from LC, here is the solution near LC, by requiring these two solutions to match in the raea where they're both valid, here are the matching conditions. 4-5 lines. Here's the presence of simple poles, gives us dispersion relation, but also here's the piece that is not divergent and containts the discontinuity. This term is not included in the sum over simple poles. Don't go through the derivation in a lot of detail. 

% WKB Approximation - could cite (Bender & Norzog)? Arfken might have it too?

% Go through derivation of dispersion relation, then say "the denominator being zero leads to the dispersion relation (cite eq 47 in time dependent section)".
