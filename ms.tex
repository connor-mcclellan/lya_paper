...
Two ~solutions~ will be considered for this problem. % Add the fact that J=J0 + Jbc. Don't refer to them as two different solutions --- they're two parts / pieces of the true solution.

a \textbf{J=0 solution} ($\rm H_0$) which includes the delta function source and satisfies $J=0$ at $r=R$,

the \textbf{bc solution} ($\rm H_{bc}$) that allows the boundary condition $J=\sqrt{3}H$ to be satisfied. 
% Incorporate into paragraph instead

%TODO: Change the voigt H to be a script H
...
%When you integrate over d\nu
\be
P(x) = \frac{16\pi^2R^2H(R, x)\Delta}{L}
\ee
% We have defined this such that Integral of P(x)dx = 1
% Mention that H is written per dnu (see appendix for exact details)
...
% Go through and tell them why you're shifting by integer multiples of tau0. Little abrupt here
