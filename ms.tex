...
Previous work by \citet{2002ApJ...567..922A,2015MNRAS.449.4336S} discussed Monte Carlo code acceleration for optical depths as large as these in a uniform plane-parallel medium with no absorption, but these calculations would need to be generalized to spherical symmetry in order to construct a model of resonant scattering in exoplanet atmospheres as discussed in Section \ref{sec:intro}. 
   ...
   Line core photons are pushed to the wings by biasing the atomic velocity distribution to high velocities so the opacity is lower and escape is more rapid.
