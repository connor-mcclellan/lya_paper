...
%% TODO: Exclude this entirely or incorporate with Phil's edit?
%To examine the relative size of the correction term $H_{\rm bc}$ as compared to $H_0$, we consider the case where $\tau_0$ is large and thus $J_{\rm bc} \gg H_{\rm bc}$. In this limit, $J_{\rm bc} \approx \sqrt{3} H_0$ from Eq. \ref{eq:bc2}. We evaluate Eq. \ref{eq:Jbc} at $r=R$ and solve for the Fourier coefficients, $A(s)$. These are then used in Eq. \ref{eq:Hbc} to find $H_{\rm bc}(R, \sigma)$. Dividing by the contribution from $H_0$, we obtain the proportionality
