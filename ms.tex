...
\label{eq:jem2}
\ee % TODO: Phil said to cite an equation from the appendix here?
...
\ee
%TODO: Put J after "specific mean intensity" from Phil's edits? What does that mean?
Here $J(r, \sigma t)$ is the formula for specific mean intensity $dE/(dA dt d\nu)$. Though it is written as a function of the photon frequency variable $\sigma$, it should be integrated over $d\nu$ to preserve its normalization. $J(n, \sigma, \omega)$ has units $dE/(dA d\nu)$. Inserting the expansion in Eq. \ref{eq:jrsigmat_expansion} in to Eq. \ref{eq:diffusion_eqn} and integrating against $\omega$, we obtain
...
\ee
where a negative sign is taken for large $+\sigma$ and a positive sign is taken for large $-\sigma$ to chose the finite solution as $|\sigma|\to 0$. Two numerical integrations are performed inward from large $|\sigma|$ to $\sigma_s$. Initial values for integration are obtained by setting $J=1$ and $dJ/d\sigma$ from Equation \ref{eq:single_j_derivative}. Integration gives $J$ and $J'$ on either side of $\sigma_s$, where a prime indicates the derivative $\partial/\partial \sigma$. By enforcing the matching conditions, Eqs. \ref{eq:matching_condition_1} and \ref{eq:matching_condition_2}, the eigenfunctions $J(n, \sigma, \omega)$ are obtained over the domain of photon frequencies $\sigma$. Since the solutions are linear in the starting conditions, only two integrations with different starting values are necessary.
...
% TODO: Shane suggests the following places too much emphasis on the number of spatial terms?
