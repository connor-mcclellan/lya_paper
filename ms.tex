...
We now wish to find an expression for the eigenfunctions $J(n, \sigma, \omega)$ in order to express the specific mean intensity $J(r, \sigma, t)$ as a sum over both spatial and frequency modes. To do so, we employ an ansatz that roughly agrees with Monte Carlo results for $x_s=0$ based on numerical calculations of this result. However, the analysis presented in Appendix \ref{app:wkb} suggests the ansatz is incomplete in the case where $x_s \neq 0$ and the solution is asymmetric about the line center. Let us define the damping rate to be $\gamma \equiv i\omega$, which is real and positive for damped solutions. At the eigenvalues $\gamma=\gamma_{nm}$, the response $J(n, \sigma, \omega)$ is resonant. Near these $\gamma_{nm}$ poles, the eigenfunctions will have the form 
\be \label{eq:jnsigmaomega_approx}
J(n,\sigma,-i\gamma) & \simeq \frac{ J_{nm}(\sigma) }{\gamma_{nm} - \gamma} + C(\gamma, \sigma),
\ee
where $C(\gamma, \sigma)$ varies slowly in $\gamma$. An application of the residue theorem gives %TODO: Problematic, but can this be improved given that we know it doesn't work?
\be
...
\begin{figure}
    ...
    \caption{Eigenfunctions $J_{nm}(x)$ for the lowest-order spatial eigenmode $n=1$, and $m=1, ..., 100$ with $x_s=0$, $a = 4.72\times 10^{-4}$ and $\tau_0=10^7$. } %TODO: Mentioned in previous caption that 'a' needs to be included. What's the solution here? Include or not? Reference?
    \label{fig:jsoln}
\end{figure}
