...
\ee
The discontinuity does not set the value of $a_1$ as that solution does not have a term linear in $\sigma$ when the source is at $\sigma_s=0$. However, we may also show that the solutions near and far from $\sigma=0$ have the same form where both functions have short-wavelength behavior. This allows us to relate the normalization coefficients $a_1$ and $a_3$, as well as quantize the phase of the solutions to determine a dispersion relation. The matching conditions are
\be
a_1 & = & |a_2| \\ 
a_3 & = & |a_2| 2^{3/2} k_c^{-1/3}, \\
\ee
and the phase is set by
\be
\frac{\kappa_n \sigma_{tp} \Delta}{k} & = & 2\pi b
\ee
to match the two solutions, where $b$ is a positive integer. Since $\sigma_{tp}$ contains the eigenvalue, we can solve the expression for $\gamma_{nm}$ to find the dispersion relation in Eq. \ref{eq:gamma_nm}.
