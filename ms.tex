...
\begin{figure}
    ...
    \caption{The scaling of numerically-obtained resonant frequencies as compared with the analytic expression in Eq. \ref{eq:gamma_nm}.}
    \label{fig:gamma_nm}
\end{figure}

TODO: The values of the $\gamma_{nm}$ resonant frequencies are calculable analytically. Their scale depends on $m$, $n$, and the factor $(a\tau_0)^{1/3}$ according to 
\be \label{eq:gamma_nm}
2**(-1/3) * np.pi**(13/6)*n**(4/3)*(m-7/8)**(2/3)*fc.clight/p.radius/(p.a * p.tau0)**(1/3.) 
\ee
The size of additional eigenfrequencies scales as a powerlaw in $m$, yet as is seen in Fig. \ref{fig:gamma_nm} the scaling is weak, requiring up to 500 $m$ to reduce the scale of $\gamma_{nm}^{\ \ -1}$ by two orders of magnitude.
